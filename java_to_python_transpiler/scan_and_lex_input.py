...
TOKEN_PATTERNS = {
    ...
    r"\/": DIVIDE_TOKEN_TYPE,
    r"%": MODULO_TOKEN_TYPE,
    r"(\d[_\d]*\d|\"d)\.(\d[_\,d]*\d|\d)?([eE][+-]?(\d[_\d]*\d|\d))?[fFdD]?": FLOAT_LITERAL_TOKEN_TYPE,
    r"([1-9]([_,\d]*\d+)?|0)[lL]?": DECIMAL_LITERAL_TOKEN_TYPE,
